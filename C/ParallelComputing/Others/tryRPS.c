...
#define MAXTHREADS 4                        // highest number of threads program can run on
...
//    char SCISSORS = 'S';
    ...
    {
        if (world_rank == 0)
            printf("Invalid Number of threads must be between 2 and %d.\n", MAXTHREADS);
               ...
               // printf(", size odd, %s", my_player_name);
                for (int m = 0; m < NUMGAMESPERROUND; m++ )
                {
                    if (world_rank % 2 == 0)
                    {
                        ...
                        sendto = (world_rank + k) % world_size;
                        if ((sendto % 2 == 1) || ((sendto % 2 == 0)&&(sendto < world_rank)))
                        {
                            if (player_count == 0 || SEESENDREPORTS)
                            {
                    ...
                    {
                        if ( (sendto % 2 == 0) || ((sento % 2 == 1)&&(sendto < world_rank)) )
                        {
                            if (world_rank >= player_count)
                                my_move[m] = randRPS();
